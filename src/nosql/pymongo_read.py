...
# SELECT id, tweet_url FROM project
# Notes:
...
records = code_collection.find(
    { "company": { "$eq": "adani" } },
    { "id": 1, "company": 1, "tweet_norm": 1, "_id": 0}
)
records_df = DataFrame(records)
print(records_df)
...
records = code_collection.aggregate([
    ...
    {"$project": {"id": 1, "company": 1, "tag_document.stance": 1, "_id": 0} }
])
records_df = DataFrame(records)
print(records_df)
